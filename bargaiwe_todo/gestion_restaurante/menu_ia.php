...
// 1. Verificamos que el local esté logueado
if (!isset($_SESSION['restaurant_id'])) { 
    header("Location: ../portal_bargaiwe.php"); 
    exit(); 
}

include 'db.php'; // Conexión tradicional
...
function insertarPlato($conn, $res_id, $sec, $nom, $desc, $pre) {
    ...
    if ($precio >= 0 && !empty($nombre)) {
        // Buscamos si la sección ya tiene un tipo asignado (entrada, plato, etc.)
...
if (isset($_POST['procesar_imagen']) && isset($_FILES['archivo_ia']) && $_FILES['archivo_ia']['size'] > 0) {
    ...
    if (strpos($tipo_archivo, 'image/') === 0) {
        ...
        // Te devuelve al menú principal de restaurantes con el mensaje de éxito
        header("Location: menu.php?exito=ia&creados=" . $creados);
        exit();
    }
}
?>
...
<body>
    <div id="loading-overlay" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); z-index: 9999; justify-content: center; align-items: center; flex-direction: column; color: white; backdrop-filter: blur(5px);">
        ...
        <h2 style="margin: 0; color: white; font-family: sans-serif;">🤖 Edubo está leyendo tu menú...</h2>
        <p style="font-family: sans-serif; opacity: 0.8;">Esto tarda unos segundos según el tamaño de la imagen.</p>
    </div>

    <div class="container">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <a href="menu.php" style="color: var(--text); text-decoration: none; font-weight: bold; background: var(--bg-panel); padding: 8px 15px; border-radius: 8px; border: 1px solid var(--border);">← Volver al Menú</a>
        ...
        <div class="card-plus">
            ...
            <form method="POST" enctype="multipart/form-data" onsubmit="document.getElementById('loading-overlay').style.display='flex';">
                <div class="drop-zone" id="drop-zone">
                    ...
                    <input type="file" name="archivo_ia" id="file-input" accept="image/*" style="display: none;" required>
    ...
    <script>
        ...
        // Al hacer clic en la zona, se abre el selector de archivos
        dropZone.addEventListener('click', () => fileInput.click());

        // Efecto visual cuando arrastras algo encima
        ...
        function mostrarVistaPrevia(file) {
            if (file) {
                // Solo permitimos imágenes ahora para mantener la "magia" visual
                if (file.type.startsWith('image/')) {
                    ...
                    reader.onload = (e) => {
                        ...
                        previewText.innerText = "✅ Imagen lista para procesar";
                    };
